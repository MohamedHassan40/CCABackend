...
router.get('/', requirePermission('hr.complaints.view'), async (req: Request, res: Response) => {
  try {
    ...
    const complaints = await (prisma as any).employeeComplaint.findMany({
      where,
      include: {
        employee: {
          select: {
...
router.get('/:id', requirePermission('hr.complaints.view'), async (req: Request, res: Response) => {
  try {
    ...
    const complaint = await (prisma as any).employeeComplaint.findFirst({
      where: {
        id,
        orgId: req.org.id,
      },
...
router.post('/', requirePermission('hr.complaints.create'), async (req: Request, res: Response) => {
  try {
    ...
    const complaint = await (prisma as any).employeeComplaint.create({
      data: {
        orgId: req.org.id,
        employeeId,
        title,
...
router.put('/:id', requirePermission('hr.complaints.edit'), async (req: Request, res: Response) => {
  try {
    ...
    const updated = await (prisma as any).employeeComplaint.update({
      where: { id },
      data: updateData,
      include: {
        employee: {
...
router.delete('/:id', requirePermission('hr.complaints.delete'), async (req: Request, res: Response) => {
  try {
    ...
    await (prisma as any).employeeComplaint.delete({
      where: { id },
    });
